...
vis_all_connected <- function(sp_object, feat_name_x='', feat_name_y='',
  ...
  for (i in 1:length(p)){
    ...
    if (vis_jaccard){
      ...
      J_comp <- reticulate::py_to_r(STopover$jaccard_and_connected_loc_(adata_sp, feat_name_x=feat_name_x, feat_name_y=feat_name_y, return_mode='jaccard', J_comp=T, return_sep_loc=F))
    }
