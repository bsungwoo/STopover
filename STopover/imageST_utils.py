...
def read_imageST(load_path=None, sp_adata_cell=None, sc_adata=None, min_counts=10, min_cells=5, sc_celltype_colname = 'celltype', 
    ...
    elif ST_type == "merfish":
        if grid_method!="cell":
            raise NotImplementedError("'grid_method' should be 'cell' when 'ST_type' is 'merfish'")
        cell_id = [cell_id_colname]
    ...
    if grid_method == "transcript":
        ...
        if 'csv' in tx_file_name:
            ...
            tx_coord_all = parquet.read_table(os.path.join(load_path, tx_file_name)).to_pandas()
            if ST_type == 'xenium':
                tx_coord_all = tx_coord_all[tx_coord_all['is_gene'] & (tx_coord_all[cell_id_colname] != 'UNASSIGNED')]
            tx_coord_all = tx_coord_all.loc[:,cell_id+[tx_xcoord_colname,tx_ycoord_colname,transcript_colname]]
